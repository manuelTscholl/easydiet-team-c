-- Map Dimensions
INSERT INTO fhv_easydiet2.ParameterDefinitionUnit (Name, Type) 
...
INSERT INTO fhv_easydiet2.ParameterDefinition (Name) 
    SELECT name FROM fhv_bls.naehrwertbezeichnung GROUP BY name;
    
-- Map Units for Nährwertbezeichnungen
INSERT INTO fhv_easydiet2.ParameterDefinitionUnits (parameterDefinitionId, parameterDefinitionUnitId)
    SELECT d.parameterDefinitionId, c.parameterDefinitionUnitId FROM fhv_bls.naehrwertbezeichnung a
        ...
        LEFT JOIN fhv_easydiet2.ParameterDefinitionUnit c ON c.name = b.bezeichnung
        LEFT JOIN fhv_easydiet2.ParameterDefinition d ON d.name = a.name;
...
INSERT INTO fhv_easydiet2.ParameterDefinitionUnits (parameterDefinitionId, parameterDefinitionUnitId)
    SELECT d.parameterDefinitionId, c.parameterDefinitionUnitId FROM (SELECT a.bezeichnung, a.dimensions_id FROM fhv_bls.whostandardparameter a GROUP BY bezeichnung) a
        INNER JOIN fhv_bls.dimension b ON a.dimensions_id = b.id
        INNER JOIN fhv_easydiet2.ParameterDefinitionUnit c ON b.bezeichnung = c.name
        INNER JOIN fhv_easydiet2.ParameterDefinition d ON d.name = a.bezeichnung;
...
--    myg/100g -> 100g
INSERT INTO fhv_easydiet2.ParameterDefinitionUnits (parameterDefinitionId, parameterDefinitionUnitId)
    SELECT a.ParameterDefinitionId, c.ParameterDefinitionUnitId FROM fhv_easydiet2.ParameterDefinitionUnits a
        INNER JOIN fhv_easydiet2.ParameterDefinitionUnit b ON a.ParameterDefinitionUnitId = b.ParameterDefinitionUnitId
        INNER JOIN fhv_easydiet2.ParameterDefinitionUnit c ON c.Name = REPLACE(b.Name, '/100g', '') AND c.Name != b.Name;
      
	  
-- Import Nahrungsmitteleintrag 
INSERT INTO fhv_easydiet2.Recipe (Name, BlsCode, Difficulty, Description, Benefits, CookInstructions, Amount, ParameterDefinitionUnitId)
    SELECT a.name, a.blscode, 3, "", "", "", 100, (SELECT ParameterDefinitionUnitId FROM fhv_easydiet2.ParameterDefinitionUnit WHERE Name='g') FROM fhv_bls.nahrungsmitteleintrag a;
    
-- Import naehrwerte
/*
 *  SQL for mapping a single column/naehrwert
    
    INSERT INTO fhv_easydiet2.NutrimentParameter (Value, ParameterDefinitionId, ParameterDefinitionUnitId, RecipeId) 
        SELECT COLUMN_NAME, c.ParameterDefinitionId, e.ParameterDefinitionUnitId, f.RecipeId FROM fhv_bls_naehrwerte  
            ...
            INNER JOIN fhv_easydiet2.ParameterDefinition c ON b.Name = c.name 
            -- get parameterdefinitionunit
            INNER JOIN fhv_bls.Dimension d ON d.id = b.dimensions_id 
            LEFT JOIN fhv_easydiet2.ParameterDefinitionUnit e ON e.Name = d.bezeichnung 
            -- get recipe
            INNER JOIN fhv_easydiet2.Recipe f ON a.SBLS = f.BlsCode') 
...
UPDATE fhv_easydiet2.NutrimentParameter a SET a.ParameterDefinitionUnitId = (
	SELECT c.ParameterDefinitionUnitId FROM fhv_easydiet2.ParameterDefinitionUnit b 
		INNER JOIN fhv_easydiet2.ParameterDefinitionUnit c ON c.Name = REPLACE(b.Name, '/100g', '') 
);	  