...
public class DietryPlanController extends Event<EventArgs> {
    ...
    /**
     * Initiats the DietryPlanController
     *
     * @param sender
     */
    ...
    /**
     * Create a new dietry plan. Throws NoPatientException if the activePastient
     * is null. Throws TimeIntersectionException if a dietplan for the timespan
     * which is specified by start and end date is allready defined.
     *
     ...
     * @throws TimeIntersectionException if illegal timespan is defined by start
     *             and enddate
    ...
    public DietryPlanData newDietryPlan(Date startdate, Date enddate,
        ...
        if (checkForTimeIntersections(startdate, enddate)) {
            ...
            // TODO PlanType TestPlan eingefÃ¼gt als Plantyp
            ...
            _dietPlanBo = new DietPlanBo("", startdate, new PlanTypeBo(toSet),
                    activeUser, activePatient.searchDietTreatmentBo(startdate,
                    enddate));
            ...
            for (int i = 0; i < timespanbo.getDuration(); i++) {
                ...
                _dietPlanBo.addTimeSpan(new TimeSpanBo(startdate, 1,
                        _dietPlanBo));
    ...
    /**
     * Searches database for all unique existing mealcodes.
     *
    ...
    /**
     * Searches an active treatment for a given timespan of a DietTreatment.
     * Returns null if there is no active DietTreatment for the specified
     * timespan.
     *
     * @param tsb
     * @return
    ...
    private DietTreatmentBo searchTreatmentForDietplan(TimeSpanBo tsb) {
        ...
        for (DietTreatmentBo dietTreatmentBo : getActivePatient().getTreatments()) {
            ...
            if (dietTreatmentBo.getStart().getTime() <= tsb.getStart().getTime()
    ...
    /**
     * Returns a list of all recipe main and sub categories without their
     * specific recipes.
     *
    ...
    /**
     * Returns a Set of CheckedRecipeVos which is a set of recipes and the check
     * values for active dietparameters and disfavors of the patient.
     *
     * @param mainCategory the bls Code of the main category in which search
    ...
    public MealData addRecipeToMeal(RecipeData rd, int day, float quantity,
        ...
        return _dietPlanBo.addRecipe((RecipeBo) rd, day, quantity, mealLineID,
                mcd);
    }
