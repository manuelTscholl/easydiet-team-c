...
public class SearchResult {
	 ...
	 * @param searchTp the search tabpane
	 * @return single instance of SearchResult
	 */
	public static SearchResult getInstance(NavigationTabPane navTab,
			TablePane searchTp) {
	...
	private void draw() {
		...
		if (_searchResultTablePane.getRows().getLength() > 0) {
			...
			_searchResultTablePane.getRows().remove(0,
					_searchResultTablePane.getRows().getLength());
		}
	...
	private boolean updateResults(Set<PatientData> newResults) {
		...
		if (matchingResults == _result.getCount()
				&& matchingResults == newResults.getCount()) {
			return true;
		}
