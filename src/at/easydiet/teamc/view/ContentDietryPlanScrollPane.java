...
/**
 * Represents the application's content dietry plan tab
 * (content_dietryPlan.bxml)
 * 
 * @author Michael
 */
public class ContentDietryPlanScrollPane extends ScrollPane implements
		Bindable, PatientDataListener {

	// class variables
	public static final org.apache.log4j.Logger LOGGER = org.apache.log4j.Logger
			.getLogger(ContentDietryPlanScrollPane.class);
	// instance variables
	 ...
	 * 
	 * @param namespace Contains all UI elements in content_dietryPlan.bxml by
	 *            id name
	...
	public void initialize(Map<String, Object> namespace, URL location,
		...
		((TablePane.Column) _namespace.get("planColumn"))
		...
		getComponentListeners().add(new ComponentListenerAdapter() {
			...
			public void sizeChanged(Component component, int previousWidth,
					int previousHeight) {
				int width = ((TablePane.Column) _namespace
						.get("parameterColumn")).getWidth();
				((TablePane.Column) _namespace.get("planColumn"))
						.setWidth(component.getWidth() - width);
			}
		});
		...
		// listener for scrolling, this listener always displays the parameter
		// area at the top
		getViewportListeners().add(new ViewportListener() {

			@Override
			public void scrollTopChanged(Viewport scrollPane,
					int previousScrollTop) {
				_parameterTable.getStyles().put("padding",
						"{top:" + scrollPane.getScrollTop() + "}");
			}
	...
	private void removePlan() {
		...
		_parameterTable.getRows().remove(0,
				_parameterTable.getRows().getLength());
		initMainTable();
		_dietWeeks = new ArrayList<DietWeek>();
	}
