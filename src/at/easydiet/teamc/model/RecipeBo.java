...
public class RecipeBo implements java.io.Serializable, Saveable, RecipeData {
	...
	public void calcParameters() {
		...
		for (RecipeIngredientBo recipeIngredientBo : this
				.getRecipeIngredientsBo()) {
			this.setTotalAmount(this.getTotalAmount()
					+ recipeIngredientBo.getIngredientAmount());
		}

		// calculate new parameter values
		for (RecipeIngredientBo recipeIngredient : getRecipeIngredientsBo()) {// all
																				// ingredients

			float brtaifactor = (recipeIngredient.getIngredientAmount() / recipeIngredient
					.getRecipe().getTotalAmount());// baserecipe to as
													// ingredient factor for
													// absolute units
			float fractionofrecipe = recipeIngredient.getIngredientAmount()
					/ _Recipe.getAmount();// fractionofrecipe needed to get the
											// right fractins of relative units

			for (NutrimentParameter parameter : recipeIngredient
					.getRecipeIngredient().getRecipe().getNutrimentParameters()) {// for
				// each parameter in the ingredient
				currParameterdefid = parameter.getParameterDefinition()
						.getParameterDefinitionId();
				if (summedIngredientParameter.containsKey(currParameterdefid)) {// if
																				// parameter
																				// already
																				// exists
					...
					try {// converting the type and then sum it
						...
						// TODO deciding addidtiv or multiplicative calculation
						// if condition /
						if (parameter.getParameterDefinition().getName()
								.equalsIgnoreCase("Alanin")) {
							int i = 2;// TODO kick this line --> not reachable
										// desipite i copied the string from
										// debugwindow
						}
						if (target.getName().contains("/")) {
							sum.setSum(sum.getSum()
					...
					idParameterMapping.put(currParameterdefid,
							getNutrimentParametersMap().get(currParameterdefid)
									.getNutrimentParameter());
				}
		...
		for (Entry<Long, ValidationSumValue> item : summedIngredientParameter
				.entrySet()) {
			NutrimentParameter tempValue = idParameterMapping
					.get(item.getKey());
		...
		if (getNutrimentParametersMap().isEmpty()) {
			...
			for (NutrimentParameterBo npb : recipeBo.getNutrimentParameters()) {
				temp = new NutrimentParameterBo(Float.toString(0),
						npb.getParameterDefinition(), npb
								.getParameterDefinition().getFirstUnit());
				getNutrimentParametersMap()
						.put(npb.getParameterDefinition()
								.getParameterDefinitionId(), temp);
				this._Recipe.getNutrimentParameters().add(
	...
	protected Map<Long, NutrimentParameterBo> getNutrimentParametersMap() {
		...
		if (_nutrimentParametersMap == null) {
			...
			for (ParameterDefinitionData pdd : SearchParameterController
					.getInstance().getAllParameterDefinitions()) {
				temppdb = (ParameterDefinitionBo) pdd;

				_nutrimentParametersMap.put(temppdb.getParameterDefinition()
						.getParameterDefinitionId(), new NutrimentParameterBo(
