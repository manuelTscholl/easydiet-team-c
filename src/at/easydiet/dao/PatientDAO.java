...
public class PatientDAO extends GenericHibernateDAO<Patient, Long> {
	...
	public static final org.apache.log4j.Logger LOGGER = org.apache.log4j.Logger
			.getLogger(PatientDAO.class);

	/**
	 * Searches patients with the defined criteria in the database via hibernate
	 * searching
	 * 
	 * 
	 * @param name1 firstname of the patient to search for should be empty if
	 *            not available
	 * @param name2 secondname of the patient to search for should be empty if
	 *            not available
	 * @param svn the insurance number of the patient to search
	 * @param birthday the birthday of the patient hours:minutes:seconds must be
	 *            0
	 * @return a empty or a filled list with the patients
	 */
	public List<Patient> findBySpecialProperties(String name1, String name2,
			String svn, Date birthday) {
		// if empty strings are comming in they must be null for searching with
		// hibernate
		svn = svn.equals("") ? null : svn;
		name1 = name1.equals("") ? null : name1;
		name2 = name2.equals("") ? null : name2;
		// TODO: the empty strings should be checked with a special string
		// utility class because we will need it more times

		// when all strings are null there is nothing to search
		if (name1 == null && name2 == null && svn == null && birthday == null)
			return new ArrayList<Patient>();

		// The next 4 blocks solve the problem that the name1(firstname) and
		// name2(lastname) are swapped in the searchstring
		...
		LOGGER.info(String.format("Searching for %s %s %s %s", "", name1,
				name2, svn, birthday));

		// the searchresult of hibernate as a List
		List<Patient> results = getSession().createCriteria(Patient.class)
				.add(firstPatientSearch).list();
		// the searchresult of hibernate as a List
		List<Patient> results2 = (getSession().createCriteria(Patient.class)
				.add(secondPatientSearch).list());

		// only adding patients which not already exists in this list
		for (Patient patient : results2) {
	 ...
	 * @return patient with the corresponding username
	 */
